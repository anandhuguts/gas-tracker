...
function TransactionSimulator() {
  ...
  return (
    <div className="w-full max-w-[441px]  bg-[#030715] border border-[#141B2A] flex justify-center rounded-2xl font-dsans px-5 ">
      <div className="w-full max-w-[372px] pt-6 pb-6">
            ...
            <div className="text-white space-y-2 text-[18px] font-medium">
              {Object.entries(costUsd).map(([chain, { gasUSD, totalUSD }]) => (
                <div key={chain} className="grid grid-cols-3 capitalize">
                  <span>{chain}</span>
                  <span>${parseFloat(gasUSD).toFixed(5)}</span>
